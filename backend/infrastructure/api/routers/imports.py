...
from backend.application.use_cases.dispatch_import import DispatchImport
...
def get_import_status(batch_id: UUID, request: Request, use_case: GetImportStatus = Depends(deps.get_import_status)):
    batch = invoke(use_case.execute, batch_id)
    with deps.get_uow_factory(request)() as uow:
        job = uow.jobs.get_for_resource("import", batch_id)
    return ImportStatusResponse(
        ...
        warnings=(batch.error_details or {}).get("warnings", []),
        error_details={"code": "import_failed"} if batch.status.value == "failed" else None,
...
def import_file(source_type: Annotated[ImportSourceType, Form()], file: Annotated[UploadFile, File()],
                request: Request, user: User = Depends(deps.require_writer)):
    try:
        ...
        if not content or len(content) > MAX_IMPORT_FILE_SIZE:
            raise HTTPException(422, detail={"code": "invalid_import_file"})
        try:
            use_case = DispatchImport(deps.get_uow_factory(request), request.app.state.background_worker.spool)
            result = invoke(use_case.execute, ImportFileCommand(
                source_type=source_type, file_name=name, content=content, imported_by=user.id,
            ))
        except IntegrityError:
            raise HTTPException(422, detail={"code": "invalid_import_data"}) from None
        request.app.state.background_worker.notify()
        return ImportResponse(batch_id=result.id, source_type=result.source_type, status=result.status,
